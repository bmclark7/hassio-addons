...
#    if [ -f /data/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.1 ]; then
    if [ -f /data/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.1 ]; then
        # tail emqx.log.*
#        erlang_log=$(echo $(ls -t /data/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.*) | awk '{print $1}')
        erlang_log=$(echo $(ls -t /data/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.*) | awk '{print $1}')
...
/data/emqx-rel/_build/emqx/rel/emqx/bin/emqx start
...
tail -f /data/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.1 &
...
IDLE_TIME=0
MGMT_CONF='/data/emqx-rel/_build/emqx/rel/emqx/etc/plugins/emqx_management.conf'
...
while [ $IDLE_TIME -lt 5 ]; do
    ...
    if curl http://localhost:${MGMT_PORT}/status >/dev/null 2>&1; then
        ...
        now_erlang_log=$(ps -ef |grep "tail -f /data/emqx-rel/_build/emqx/rel/emqx/log/erlang.log" |grep -v grep | sed -r "s/.*tail -f (.*)/\1/g")
        new_erlang_log="$(ls -t /data/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.* | head -1)"
