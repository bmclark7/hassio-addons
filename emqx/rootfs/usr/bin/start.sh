...
emqx_exit(){
    ...
    if [ -f /root/.git/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.1 ]; then
        # tail emqx.log.*
        erlang_log=$(echo $(ls -t /root/.git/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.*) | awk '{print $1}')
...
/root/.git/emqx-rel/_build/emqx/rel/emqx/bin/emqx start
...
tail -f /root/.git/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.1 &
...
IDLE_TIME=0
MGMT_CONF='/root/.git/emqx-rel/_build/emqx/rel/emqx/etc/plugins/emqx_management.conf'
...
while [ $IDLE_TIME -lt 5 ]; do
    ...
    if curl http://localhost:${MGMT_PORT}/status >/dev/null 2>&1; then
        ...
        now_erlang_log=$(ps -ef |grep "tail -f /root/.git/emqx-rel/_build/emqx/rel/emqx/log/erlang.log" |grep -v grep | sed -r "s/.*tail -f (.*)/\1/g")
        new_erlang_log="$(ls -t /root/.git/emqx-rel/_build/emqx/rel/emqx/log/erlang.log.* | head -1)"
